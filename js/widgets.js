...
/* ── Calculator logic ────────────────────────────────────────────
   Wired to the buttons in _injectCalc(). Maintains an expression
   string, evaluates safely on '=', and supports basic scientific
   functions (sqrt, x², 1/x, sin/cos/tan in degrees, log, ln, π, |x|, %, ±).
─────────────────────────────────────────────────────────────────── */
var _CALC = { expr: '', last: '0' };
...
function _calcRender() {
  var ex = document.getElementById('calc-fab-expr');
  var rs = document.getElementById('calc-result');
  if (ex) ex.textContent = _CALC.expr || '';
  if (rs) rs.textContent = _CALC.last || '0';
}

function calcNum(ch) {
  _CALC.expr += String(ch);
  _calcRender();
}

function calcOp(op) {
  if (!_CALC.expr && _CALC.last) _CALC.expr = String(_CALC.last);
  if (!_CALC.expr) return;
  var lastChar = _CALC.expr.slice(-1);
  if ('+-*/'.indexOf(lastChar) !== -1 && '+-*/'.indexOf(op) !== -1) {
    _CALC.expr = _CALC.expr.slice(0, -1) + op;
  } else {
    _CALC.expr += op;
  }
  _calcRender();
...
function calcFn(fn) {
  var current = _CALC.expr || _CALC.last || '0';
  var n = parseFloat(current);
  var r;
  if (fn === 'pi') { _CALC.expr += String(Math.PI); _calcRender(); return; }
  if (isNaN(n)) {
    // try evaluating expression first
    try { n = _calcEval(current); } catch(e) { return; }
  }
  switch (fn) {
    case 'sqrt': r = Math.sqrt(n); break;
    case 'sq':   r = n * n; break;
    case 'inv':  r = 1 / n; break;
    case 'sin':  r = Math.sin(n * Math.PI / 180); break;
    case 'cos':  r = Math.cos(n * Math.PI / 180); break;
    case 'tan':  r = Math.tan(n * Math.PI / 180); break;
    case 'log':  r = Math.log10 ? Math.log10(n) : Math.log(n) / Math.LN10; break;
    case 'ln':   r = Math.log(n); break;
    case 'abs':  r = Math.abs(n); break;
    case 'pct':  r = n / 100; break;
    case 'pm':   r = -n; break;
    default: return;
  }
...
function _calcEval(s) {
  // Sanitize: allow only digits, decimal point, basic operators, parens, spaces
  var clean = String(s).replace(/\s+/g, '');
  if (!/^[0-9.+\-*/()]+$/.test(clean.replace(/\*\*/g, ''))) {
    throw new Error('expr inválida');
  }
...
function _calcFmt(n) {
  ...
  // Round to 10 decimals to avoid floating point noise, then strip trailing zeros
