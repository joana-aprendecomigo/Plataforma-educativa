...
function buildEx3(tema,tipo,dif){
  ...
  if(tema==='2'){
    ...
    if(variant===0){
      ...
      return{tema:'Tema 2',tipo:'mc',
        ...
        opcoes:sh3([ext].concat(w)),resposta:ext,expl:'â_ext = 360° ÷ '+n+' = '+ext+'°.'};
    }
    if(variant===1){
  ...
  if(tema==='3'){
    var ang=rnd3(easy?30:15, easy?150:165);
    var supl=180-ang;
    var variant=rnd3(0,4);
    if(variant===0){
    ...
    if(variant===2){
      return{tema:'Tema 3',tipo:'mc',
        ...
        opcoes:sh3([ang,supl,90,180-ang+10].filter(function(v,i,a){return a.indexOf(v)===i&&v>0&&v<180;}).slice(0,4)),
  ...
  if(tema==='5'){
    ...
    if(fig===0){
      ...
      return{tema:'Tema 5',tipo:'mc',
        ...
        opcoes:sh3([a].concat(w)),resposta:a,expl:'A = ('+b+' × '+h+') / 2 = '+a+' cm².'};
    }
    if(fig===1){
      ...
      return{tema:'Tema 5',tipo:'mc',
        ...
        opcoes:sh3([a].concat(w)),resposta:a,expl:'A = '+b+' × '+h+' = '+a+' cm².'};
    }
    if(fig===2){
      ...
      return{tema:'Tema 5',tipo:'mc',
        ...
        opcoes:sh3([a].concat(w)),resposta:a,expl:'A = ('+b1+'+'+b2+')/2 × '+h+' = '+a+' cm².'};
    }
    if(fig===3){
      ...
      return{tema:'Tema 5',tipo:'mc',
        ...
        opcoes:sh3([a].concat(w)),resposta:a,expl:'A = ('+d1+'×'+d2+')/2 = '+a+' cm².'};
    }
    if(fig===4){
...
function calcArea3(){
  ...
  var area=0,formula='';
  if(fig==='triangulo'){var b=g('ar-triangulo-b'),h=g('ar-triangulo-h');area=b*h/2;formula='('+b+'×'+h+')/2 = '+area;}
  else if(fig==='paralelogramo'){var b=g('ar-paralelogramo-b'),h=g('ar-paralelogramo-h');area=b*h;formula=b+'×'+h+' = '+area;}
  else if(fig==='trapezio'){var b1=g('ar-b1'),b2=g('ar-b2'),h=g('ar-trapezio-h');area=(b1+b2)/2*h;formula='('+b1+'+'+b2+')/2×'+h+' = '+area;}
  else if(fig==='losango'){var d1=g('ar-d1'),d2=g('ar-d2');area=d1*d2/2;formula='('+d1+'×'+d2+')/2 = '+area;}
  else if(fig==='circulo'){var r=g('ar-circulo-r');area=parseFloat((Math.PI*r*r).toFixed(2));formula='π×'+r+'² ≈ '+area;}
  else if(fig==='semicirculo'){var r=g('ar-semicirculo-r');area=parseFloat((Math.PI*r*r/2).toFixed(2));formula='(π×'+r+'²)/2 ≈ '+area;}
